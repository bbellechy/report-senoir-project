...
หรือ แผนภาพเส้นทางผู้ใช้ เป็นเครื่องมือที่ใช้อธิบายขั้นตอนและประสบการณ์ของผู้ใช้เมื่อมีปฏิสัมพันธ์กับแอปพลิเคชันของเรา ว่าผู้ใช้โต้ตอบอย่างไร พบปัญหาตรงไหน และจะปรับปรุงจุดใดได้บ้าง เพื่อให้ผู้ใช้ได้รับประสบการณ์ที่ดีขึ้น ซึ่งจะแสดงภาพรวมตั้งแต่จุดเริ่มต้นที่ผู้ใช้เริ่มสนใจไปจนถึงการบรรลุเป้าหมายโดยแบ่งเป็นช่วงเวลาและเหตุการณ์สำคัญต่าง ๆ โดยมีองค์ประกอบ ดังนี้\cite{journey}
...
รวบรวมฟังก์ชันที่ผู้ใช้ต้องการ ได้แก่
...
กำหนดคุณสมบัติที่ระบบต้องมี ได้แก่
...
ข้อมูลต้นทางมาจาก สำนักพัฒนามาตรฐานระบบข้อมูลสุขภาพไทย \cite{tmt2013} ซึ่งเผยแพร่ข้อมูลในรูปแบบไฟล์ CSV ข้อมูลดังกล่าวจะถูกนำมาใช้เป็นแหล่งข้อมูลหลักของระบบ

\subsubsection{Data Processing}
ข้อมูลจากไฟล์ CSV จะถูกนำเข้าสู่กระบวนการประมวลผลด้วย PySpark \cite{pyspark2022} ซึ่งเป็นเครื่องมือที่เหมาะสำหรับการจัดการและประมวลผลข้อมูลขนาดใหญ่ (Big Data Processing) เพื่อจัดรูปแบบข้อมูลให้อยู่ในโครงสร้างที่เหมาะสมต่อการจัดเก็บและการนำไปใช้งานต่อไป
...
\subsubsection{System Deployment (kind Cluster)}
สำหรับการทดสอบและจำลองสภาพแวดล้อมการทำงานของระบบบน Kubernetes ได้มีการใช้เครื่องมือ kind \cite{kind2025} (Kubernetes IN Docker) เพื่อสร้าง Kubernetes cluster จำลองบนเครื่องพัฒนา (Local Environment)

โดย cluster ที่สร้างขึ้นจะประกอบด้วย container หลัก ได้แก่

\begin{itemize}[leftmargin=4em]
    \item Spring Boot Application Container
    \item PostgreSQL Database Container
\end{itemize}
...
อธิบายโครงสร้างระหว่างฐานข้อมูลในรูปแบบแผนภาพ โดยในโครงงานนี้ จะใช้ในแนวคิด ERD แบบ Crow’s Foot Notation ซึ่งจะประกอบไปด้วย \cite{erd2022}
\begin{itemize}[leftmargin=2em]
...
\subsubsubsection{สัญลักษณ์}
\begin{itemize}[leftmargin=2em]
    \item \textbf{○ (วงกลม):} Zero ซึ่งสามารถไม่มีค่าได้
    \item \textbf{< (ขาแฉกสามง่าม):} Many หมายถึงมีได้หลายรายการ
    \item \textbf{○<:} Zero or Many หมายถึงอาจไม่มีค่าได้หรือมีได้หลายรายการ
    \item \textbf{1 (ขาแฉกเดียว):} One หมายถึงมีได้หนึ่งรายการเท่านั้น
    \item \textbf{○1:} Zero or One หมายถึงอาจไม่มีค่าได้หรือมีได้หนึ่งรายการเท่านั้น
\end{itemize}
...
\paragraph{Manage User Information}
\begin{figure}[!htb]
    \centering
...
\paragraph{OCR/ Barcode/ Text-To-Speech}
\begin{figure}[!htb]
    \centering
...
\paragraph{Medication Management}
\begin{figure}[!htb]
    \centering
...
\paragraph{Notification Management}
\begin{figure}[!htb]
    \centering
...
\begin{itemize}[leftmargin=2em]
    ...
    \begin{itemize}[leftmargin=2em]
        ...
        \item \texttt{flutter\_tesseract\_ocr:} สำหรับฟังก์ชัน OCR
...
\subsubsection{การพัฒนา UI/UX}
ออกแบบและสร้างหน้าจอการใช้งานตามหลัก Human-Computer Interaction (HCI) ที่เหมาะสมกับผู้สูงอายุ เช่น ใช้ขนาดตัวอักษรที่ใหญ่, สีสันที่ตัดกันชัดเจน และมีปุ่มกดที่ชัดเจน

\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/chap4/figma/figma.png}
    \captionsetup{name=รูป}
    \caption{ออกแบบ UI/UX ของ Application}
    \label{fig:ui_design}
\end{figure}

\subsubsection{การพัฒนา API Backend}
สร้าง RESTful API ด้วย Spring Boot เพื่อจัดการคำขอจากแอปพลิเคชัน (Client) และเชื่อมต่อกับฐานข้อมูล PostgreSQL
